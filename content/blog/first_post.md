---
title: my first class blog
description: this is my submission for in-class #2
date: 2026-02-04
---
<!--above is llty syntax for metadata of the page (always put at the very top of each .md file ^^)-->

# this is a heading with one '#'

if you hate markdown, you can just qrite plain text and that is totally fine this i sjust a test file

press enter for new paragraph

<img src="https://pbs.twimg.com/media/EpytfxlXUAA6Fx6.jpg">

loo loo la la la la la 
